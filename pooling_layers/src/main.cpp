...
#include<Eigen/Dense>
template <typename T>
const Eigen::Tensor<T,2> pooling(const Eigen::Tensor<T,2> &X, int pool_size, int strides = 1) {
    ...
    // getting the patches
    Eigen::Tensor<T,5> patches = reshaped.extract_image_patches(pool_size, pool_size, strides, strides, Eigen::PADDING_VALID);
